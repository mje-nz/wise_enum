...
void setup() {
    Serial.begin(9600);  // Teensy serial is always full USB speed, baud rate is ignored
    while (!Serial) {};  // Wait for serial monitor

    // Number of enumerations:
    static_assert(wise_enum::size<my_lib::Color> == 2, "");
    Serial.print("Number of enumerations: ");
    ...
    for (auto e : wise_enum::range<MoreColor>) {
        Serial.print(static_cast<int>(e.value));
        Serial.print(" ");
        Serial.println(e.name);
    }
    for (auto e : wise_enum::range<Bar::Foo>) {
        Serial.print(static_cast<int>(e.value));
        Serial.print(" ");
        Serial.println(e.name);
    }
    ...
    Serial.println(static_cast<int>(x1));
    Serial.println();
